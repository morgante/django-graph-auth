...
from django.contrib.auth.tokens\
    import default_token_generator as token_generator
...
class UserNode(DjangoObjectType):
    ...
    def get_node(cls, id, context, info):
        ...
        if context.user.id and (
                user.id == context.user.id or context.user.is_staff):
            return user
        else:
            return None
    ...
    def resolve_token(self, args, context, info):
        if self.id != context.user.id and\
...
class RegisterUser(relay.ClientIDMutation):
    ...
    def mutate_and_get_payload(cls, input, context, info):
        ...
        if graph_auth_settings.ONLY_ADMIN_REGISTRATION and not (
                context.user.id and context.user.is_staff):
        ...
        if graph_auth_settings.WELCOME_EMAIL_TEMPLATE is not None\
            ...
            message = EmailMessage(
                graph_auth_settings.WELCOME_EMAIL_TEMPLATE, input_data,
                graph_auth_settings.EMAIL_FROM, [user.email])
            message.send()
